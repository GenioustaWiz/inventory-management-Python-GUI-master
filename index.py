...
def Database():
    ...
    cursor.execute("SELECT * FROM `employee` WHERE `username` = 'admin1' AND `password` = 'admin1'")
...
def Exit2():
    result = tkMessageBox.askquestion('Inventory Management System', 'Are you sure you want to exit?', icon="warning")
    if result == 'yes':
        root.deiconify() #bring root window to visibility
        rtn = Adm_Home.destroy()  
        exit()
    """"
    def ShowLoginForm():
    global loginform
...
def Adm_Home():
    global Home
    Home = Tk()
    Home.title("Inventory Management System/Inventory")
    width = 1024
    height = 520
...
def Emp_Home():
    global Home
    Home = Tk()
    Home.title("Inventory Management System/Sales")
    width = 1024
    height = 520
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
    ...
    filemenu2.add_command(label="View", command=ShowView)
    menubar.add_cascade(label="Account", menu=filemenu)
    menubar.add_cascade(label="Sales", menu=filemenu2)
    Home.config(menu=menubar)
    Home.config(bg="White")
...
login_dis = Frame(root, bd=1, relief=SOLID)
login_dis.pack(pady=10)

#========================================LABEL WIDGET=====================================
lbl_display = Label(Title, text="Welcome to IRS", font=('arial', 45))
lbl_display.pack()

lbl2_display = Label(Title, text="Am a Retail & Inventory Management System, here to make your Business Life Easy", font=('arial', 12))
lbl2_display.pack()


btn_admin = Button(login_dis, text="Admin Login", command=Admin_LoginForm, font=('arial', 20) )
btn_admin.pack(side=TOP, padx=10, pady=10)
btn_empl = Button(login_dis, text="Employee Login", command=Emp_LoginForm, font=('arial', 20))
